...
- 🔌 **协同编辑**：通过 `@collaflow/core` 的 Hocuspocus 协同服务实现多人实时同步
- 🎨 **主题变量**：内置明暗主题 CSS 变量
...
> 服务端需部署 `@collaflow/core`（Hocuspocus 协同服务，默认 `ws://localhost:1234`），客户端通过 `serverUrl` 连接，`roomId` 作为文档名。
